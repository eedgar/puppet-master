class cobbler::centos {
        ...
        exec { centos55-updates-i386 :
                command => "/usr/bin/cobbler repo add --name=centos5.5-i386-updates --priority=70 --mirror=http://mirror.cs.vt.edu/pub/CentOS/5.5/updates/i386/ --mirror-locally=0",
                logoutput => true,
                unless => "/usr/bin/cobbler repo list|grep centos5.5-i386-updates; test $? -eq 0",
        }
        exec { EPEL-i386 :
                command => "/usr/bin/cobbler repo add --name=EPEL-i386 --priority=40 --mirror=http://download.fedora.redhat.com/pub/epel/5/i386 --mirror-locally=0",
                logoutput => true,
